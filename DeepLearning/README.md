...
神经网络的训练主要目的是为了获取神经网络最优权重参数（权重和偏置）。

在最初的方法，对于一个问题，需要人们想出解决办法来得到答案。

在机器学习中，由人设置特征量输入给机器学习模型由模型根据人们设定的特征量来进行学习最终得到答案。

在深度学习中，由机器自己从数据中进行学习得到答案。

从理论上来说，神经网络可以拟合出任意函数。

数据分为训练集和测试集（分数据的方法：自助法，交叉验证法，保留法）。

模型在训练集上进行学习，训练得到的参数，再测试模型在测试集上的表现（泛化能力：对于未训练新样本的适应性）。

模型预期的能力和预期的能力之差叫做误差。

训练出来的模型和模型真实水平之间的差值叫做偏差。

噪声是偏差的下限。

神经网络在训练过程中容易出现欠拟合和过拟合。

欠拟合主要是你将树叶输入到模型里面训练，但是训练出来的模型无法正确识别出树叶，欠拟合主要是因为训练轮数过少导致的。

过拟合则主要是因为过度训练导致将个体的独有的特征识别成树叶这一类事物的特征。目前过拟合也是神经网络训练的主要问题。

类别不平衡问题：指的是分类任务中不同类别的训练样例数目差别较大的情况

对样例中多里数目样例进行欠采样，丢掉一些多的样例，让正反样例数目相近，在进行学习 对样例汇总数目少的样例进行过采样，增加一些样例少的一方的样例，在进行学习 直接基于原始训练集进行学习，但在用训练好的分类器进行预测时进行阈值移动

过采样：过采样法不能简单地对初始样本进行重复采样，否则容易导致过拟合

过采样代表性的算法：SMOTE算法，通过对训练集中的样例进行插值来产生额外的正例

​欠采样：欠采样法若随机丢弃，可能丢失一些重要信息。

欠采样法的代表性算法：EasyEnsemble算法，通过集成学习机制，将样例划分若干集合供学习器使用。 对每个学习器欠采样，但是对全局不产生影响。

为了更好的找到神经网络的参数，我们引入了损失函数的概念，损失函数可以使用任意函数，但是一般用均方误差和交叉熵误差等。

均方误差在回归类问题中也是非常重要的参考数值。

均方误差会计算神经网络的输出和正确解之间各个元素之差的平方，再求和。

我们主要希望均方误差最小化，来达到整合网络的参数能够让网络的输出结果和正确解之间最为接近。

我们希望得到一套损失函数最小的权重参数，那么我们就需要不断往损失函数小的方面进行努力，而根据导数（梯度）来判断参数优化方向使我们经常用的方案。

我们经常使用的一些优化方法：随机参数法、随机梯度下降法，模拟退火，遗传算法。

这些算法能够避免陷入局部最小值当中。这是很重要的，因为有的时候我们会因为局部最小值的存在而导致模型无法达到最优性能。

很多人把神经网络的搭建过程称为“搭积木”，而把神经网络的优化过程称为“炼丹”

神经网络的初始值一般都是人们靠经验设置的，根据第一轮优化结果再对神经网络参数进行调整。

神经网络的参数由两种，向学习率这类的参数（超参数）是由人们手动设置的，对于特征量的参数（参数）是由模型从数据中学习得到的。

神经网络训练的流程：设置初始值->模型训练->获得参数->评价参数(计算梯度)->更新参数
